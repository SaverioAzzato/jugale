      ...
      sections: [
        ...
        },
        {
          ...
          bullets: [
            ...
            "Tap the large floating die at the bottom right whenever you need a roll.",
            "Use + condition and Inspiration in Play to keep the current situation visible.",
          ],
        },
        {
          title: "Roll and move dice",
          intro: "The large dice button floats at the bottom right by default. In Settings → Dice button you can move it to the bottom left or put it in the top bar.",
          bullets: [
    ...
    },
    {
      ...
      sections: [
        ...
        },
        {
          ...
          intro: "To send the material immediately, tap Share and choose the chatbot. You can use Download instead as described below: it is available on Android too. When the chatbot has finished, download the complete character.json. In Files or Downloads, share that file and choose JUGALE or JUGALE Dev. JUGALE always shows a preview before changing anything.",
        ...
        {
          title: "Download one file on any device",
          intro: "Download is available on web, desktop and Android. Use it when you want to attach one file to the chatbot instead of sharing directly from the app.",
          steps: ["Open Prompts and choose the task.", "Select Download beside that prompt. JUGALE saves a text file with the prompt and current JSON Schema.", "If a character is open, the file also includes its complete character.json. Migrate additionally includes the schema changelog.", "Attach the text file in your chatbot conversation.", "Save the final answer as character.json, then import it as described below."],
          bullets: ["You can use Download without opening a character: the file still contains the prompt and schema, but omits character.json."],
        },
        {
          title: "Import the returned character",
          intro: "Use Import character JSON in the top bar on web, desktop or Android. JUGALE previews the selected file before saving or replacing anything.",
          steps: ["Save the chatbot's complete answer as a .json file.", "Select Import character JSON and choose that file.", "Check the character name, schema version, errors and warnings in the preview.", "If a character is open, choose Apply to replace its character.json. Existing images stay in place; when version history is enabled, JUGALE first saves the current character in Versions.", "If no character is open, choose Character folder and select a truly empty folder. JUGALE creates character.json there only after you confirm."],
          bullets: ["Apply and Create stay disabled until the selected JSON is valid and supported by this JUGALE version.", "Creating a character folder enables folder images and Versions immediately. A non-empty folder without character.json is rejected.", "Writable folder creation is available on Chromium-based browsers, desktop and Android; other browsers explain the limitation."],
        },
      ],
    },
    {
