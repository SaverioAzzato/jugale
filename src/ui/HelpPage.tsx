...
const CONTENT: Record<Locale, HelpContent> = {
  en: {
    ...
    miscItems: [
      ...
      "The welcome screen keeps a Recent list so you can reopen a character in one click (where the platform allows it — desktop and Chromium browsers). It's purely local: only a reference to the file is stored on your device, the contents are never sent anywhere. Clear it anytime with Clear, and an entry that no longer opens (file moved or deleted) is dropped automatically.",
      "Everything stays on your device: JUGALE has no server and no tracking, and sends nothing anywhere. Your character lives in its file, and recents/settings/theme are saved locally in your browser or app only.",
  ...
  it: {
    ...
    miscItems: [
      ...
      "La schermata iniziale tiene un elenco Recenti per riaprire un personaggio con un clic (dove la piattaforma lo permette — desktop e browser Chromium). È tutto locale: sul tuo dispositivo viene salvato solo un riferimento al file, il contenuto non viene mai inviato da nessuna parte. Puoi svuotarlo quando vuoi con Svuota, e una voce che non si apre più (file spostato o eliminato) viene rimossa in automatico.",
      "Tutto resta sul tuo dispositivo: JUGALE non ha server né tracciamento e non invia nulla da nessuna parte. Il personaggio vive nel suo file, e recenti/impostazioni/tema sono salvati solo in locale nel browser o nell'app.",
