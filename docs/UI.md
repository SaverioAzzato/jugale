...
- **Spells** — organized by `spellSections[]` (cantrips / by level / rituals / …). Each spell is a collapsed row (the numbers you say at the table: range · to-hit or save DC · damage, plus `ritual`/concentration tags) → tap → rich card (school, casting time, area, duration, concentration, V·S·M components, the material list with cost/consumable, damage + type, "at higher levels", a multi-line **description**, and the wiki **link**). Spell save DC / attack shown once.
...
- Settings also persists the **Dice button** position: floating at the bottom right (default), floating at the bottom left, or in the top bar. The floating variants use a larger 56 px circular touch target; the compact toolbar variant keeps the standard toolbar size. Floating placement respects system safe areas and remains available on full-page app views.
- Every icon-only button exposes the same localized label to assistive technology and as a native mouse-hover hint. The character toolbar measures its rendered space after Interface scale is applied. When actions no longer fit, lower-priority ones move into **More (`…`)** in this disappearance order: Settings, Help, Import, Export, Versions, Save version, prompts, Raw JSON, Edit, dice. Import remains available on the welcome screen and has priority there over prompts and Settings. Version actions are omitted when unavailable; dice participates only when its selected position is the top bar. The Back button is independent and always remains visible.
- Thrown dice use visual-pixel DOM measurements after Interface scale and browser zoom. Their centers are constrained so the complete sticky app bar (toolbar plus tabs), the conditional bottom status bar, and the active floating dice button are expanded by the die radius into collision obstacles. Existing dice reflow when either bar appears or changes size. The menu and both dice-button placements share a stacking layer above the WebGL canvas, so the selector and app chrome are never painted behind resting dice.
...
- **AC.** The renderer does **not** know AC rules. Each armor/shield **item declares its own AC contribution** (base + Dex handling + bonus/malus). The base comes from the one worn body armor (or `10 + Dex` when unarmored), then every equipped `bonus` (shield, ring) stacks on top; the AC value carries a **provenance note** — e.g. `cuoio 11 + des 3`, `no armor + shield +2`, `ombra` (Warlock shadow armor). A manual `combat.armorClassOverride` always wins. Only one body armor may be equipped at a time — the Equip button locks the others (and imported data with two is flagged). Equipping/unequipping recomputes from the declared data. **GPT prompts (M3) must be instructed carefully to maintain these encodings** — noted in ROADMAP M3.
- **Resources & rests.** Rest mechanics are uniform across classes in 5e RAW; what varies is *which* resources reset, declared per-resource via `resetOn` (`shortRest`/`longRest`/`dawn`/`manual`/`none`). The rest buttons are a convenience macro applying these to the declared data (+ HP/Hit Dice rules on long rest) — **no class logic in code**, and the user can always adjust manually. **Hit Dice** live on `combat.hp.hitDiceRemaining` (live), with the max derived from total level (`maxHitDice`) — kept on HP rather than as a generic resource, matching the existing schema.
