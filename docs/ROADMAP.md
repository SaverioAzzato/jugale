...
- **Tauri desktop shell ✅ done:** `src-tauri/` (identifier `it.azzato.jugale`, app icon generated from the `:J` brand mark via `tauri icon`) wraps the same web build with no frontend changes beyond storage. `src/storage/tauriProvider.ts` implements the `StorageProvider` interface natively — `@tauri-apps/plugin-dialog` for the native open-file/open-folder pickers, `@tauri-apps/plugin-fs` for read/write and `images/` directory scanning — and `App.tsx` picks it over the browser File System Access path via a runtime `isTauri()` check. Desktop always has live read/write (no import/export fallback needed, unlike the browser's non-Chromium path). `.github/workflows/tauri-build.yml` compiles + bundles on a Win/Mac/Linux matrix on every push/PR and uploads the bundles as CI artifacts. Verified locally end-to-end: `npm run tauri build` produces a working `.app`, launches, stays running, and quits cleanly.
- Android APK via CI (mobile shell, separate follow-up — shares most of the Tauri config above).
- **GitHub Pages web build ✅ done:** [`pages.yml`](../.github/workflows/pages.yml) builds and deploys `dist/` on every push to `main` (and on demand via `workflow_dispatch`). No SPA-routing fallback needed (no client router). Import/export and the "you'll lose changes" unload guard (`src/App.tsx`'s `beforeunload` handler) already existed from earlier milestones. Repo is public, Pages source is set to "GitHub Actions", and the site is live at https://saverioazzato.github.io/jugale/.
...
M0–M3 are done; M4's web build and Tauri desktop shell are done. What's left in M4: the Android mobile shell (shares most of the desktop Tauri config) and `release.yml` (attaching built artifacts to GitHub Releases on tag).
