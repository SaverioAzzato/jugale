...
The shipped prompts follow the app's UI language (English or Italian) — switch the language and untouched prompts switch with it; once you Save an edit, that set is frozen exactly as written until Reset. A separate **Custom instruction** field appends your own standing instruction to the base of every composed prompt (also per-browser).
